...
func _reveal_chosen_cards() -> void:
	...
	for player_index in range(1, num_players + 1):
		var number = chosen_cards[player_index]
		var card = CardScene.instantiate()
...
func _place_card(played_number: int) -> void:
	...
	if best_row_index == -1:
		# Low card rule
		var chosen_row_index = _choose_row_for_player(current_player)
		var taken_row = stacks[chosen_row_index]
		print("Player %d takes row %d: %s" % [current_player, chosen_row_index + 1, str(taken_row)])
